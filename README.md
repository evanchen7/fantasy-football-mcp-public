...
- Same-season catalog `rank_adp` is used only for known `STD`, and `rank_adp_ppr` only for known `PPR`. For `HALF`, the service requests FantasyPros consensus rankings with `type=ADP`, `scoring=HALF`, `position=ALL`, and `week=0`, then retains only exact player identity plus `rank_ave`. It never interpolates STD/PPR values or substitutes ECR for ADP.
...
The first FantasyPros-enabled recommendation automatically fetches the base player catalog, injuries, news, one scoring-specific projection snapshot, and—when needed—the scoring-specific ADP snapshot, then populates the shared provider cache at `~/.fantasy-football-mcp/provider-snapshots.sqlite3`. FantasyPros and Sleeper snapshots are cached for 24 hours; injury/news snapshots retry after five minutes. On first use, an existing `fantasypros-snapshots.sqlite3` is copied atomically into the provider-neutral path and its schema is migrated transactionally while preserving valid rows; the legacy file is retained as a recovery copy. No separate command is needed. The SQLite cache is bounded to sixteen snapshot variants and 8 MB of normalized record JSON. It contains normalized allowlisted evidence only—not the API key, raw provider bodies, URLs, query strings, targeted identity lookups, Yahoo data, draft state, league IDs, or recommendation candidates. User-facing warnings identify stale fallback; stale injury status/headlines remain suppressed and stale projection/ADP provenance remains explicit.

## Sleeper player experience cache

No Sleeper API key is required. When fresh FantasyPros preseason projections are present, the service requests `https://api.sleeper.app/v1/players/nfl?active=true`, retains only RB/WR/TE identity fields plus `years_exp`, and caches that normalized catalog for 24 hours in `~/.fantasy-football-mcp/provider-snapshots.sqlite3`. Network ingestion is capped at 16 MiB and 12,000 source records; the allowlisted normalized catalog remains capped at 10,000 records. Exceeding any of those bounds fails closed. The shared database keeps each provider snapshot under a separate allowlisted key. An existing `sleeper-players.json` is validated, inserted into SQLite on first use, and removed only after that write succeeds. The directory and database are restricted to `0700` and `0600` respectively. A failed refresh may use a cache no older than 45 days; older, malformed, oversized, or ambiguous data fails closed. The raw Sleeper response, unrelated player fields, and request URLs are not persisted.
...
FantasyPros receives only its API requests. No draft ledger or Yahoo credential is sent to FantasyPros. Sleeper receives only one generic active-NFL-player catalog request; no ranking list, league identity, or draft state is sent. When the Databricks advisory critic is explicitly enabled, Databricks receives only the identity-free allowlist described above; this app keeps its advisory cache in memory only. Google Drive receives nothing from this app because it is not a runtime integration.
