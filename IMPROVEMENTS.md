...
- Popup ledger diagnostics with exact missing, duplicate, and sanitized unnumbered-pick details, plus guarded authoritative full repair that distinguishes strictly numbered pre-rendered future rows using stable current-pick evidence. Recommendation-ready state requires an allowlisted Round-by-Round proof; an empty pick-1 ledger additionally requires stable current-pick evidence. Unsafe, ambiguous, or observation-free active capture persists a privacy-safe per-league server blocker until a verified coherent scan or accepted repair clears it, while repeated empty scans cannot refresh stale state indefinitely.
...
- Dashboard-only provider-cache maintenance with metadata-only database statistics, per-dataset freshness, local FantasyPros request reservations, and one non-queueing TTL-aware FantasyPros + Sleeper warm job scoped to an explicit scoring format. The endpoints require loopback client/host checks, a same-origin mutation request, bounded exact JSON, and never expose cache paths, player rows, provider bodies, credentials, or draft identity.
- Exact-session local DraftSheets/JSON profile import with bounded parsing, private atomic storage, explicit `STD`/`HALF`/`PPR` selection, zero Yahoo API calls when a matching profile is available, and current-season per-sport source-plus-scoring defaults for future profileless recorder drafts without overwriting exact profiles or copying picks. Older scoring-less profiles/defaults remain readable and visibly upgradeable.
...
- Official scoring-neutral FantasyPros preseason RB/WR/TE projection feeds with strict local `STD`/`HALF`/`PPR` points-field selection, cached scoring-specific allowlisted views, and normalized opportunity volume; same-season explicit STD/PPR catalog ADP; cached official HALF-PPR consensus ADP; a 10-second enrichment deadline; private bounded cache migration; and stale provenance. HALF-PPR uses the provider's `type=ADP` consensus response, including validation of its `ADP Half PPR` label, rather than interpolation or ECR substitution. Conservative matches against a normalized Sleeper NFL player catalog add genuine `years_exp` evidence, cached privately for 24 hours with a 45-day stale fallback, so complete FantasyPros + Sleeper cohorts can produce Breakout Watch labels without changing deterministic scores. Cross-provider matching now prefers an authoritative Yahoo ID plus position, carries FantasyPros' validated consensus `player_yahoo_id` only through that join, and reports bounded match-tier counts for eligible RB/WR/TE ranking rows. Unique exact name/position/team matches include exact initialed names; non-initialed fallbacks remove at most one terminal generational suffix with the same position/team or accept only an exact `FA`-versus-missing-team difference. Stable-ID failures, team conflicts, and ambiguity never fall back loosely. Sleeper ingestion accepts at most 16 MiB and 12,000 source records while preserving the 10,000-record normalized cache bound; oversized input fails closed and raw responses are never persisted. Both providers share a provider-neutral SQLite cache; legacy FantasyPros SQLite and Sleeper JSON stores migrate automatically. A metadata-only CLI can warm or explicitly refresh the Sleeper snapshot for scheduled jobs.
