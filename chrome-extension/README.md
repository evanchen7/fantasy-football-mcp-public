...
8. Open Firefox's **Draft Assistant** sidebar or remain in the dashboard. Live recommendation refresh starts automatically; **Refresh recommendations** remains available for an immediate manual run.
9. When diagnosing any recommendation, inspect live draft state before requesting a next pick.

The smaller Yahoo last-pick banner and the active Yahoo **Picks** tab continue supplying new observations while other draft panels are visible. DOM bursts use a 400 ms quiet window, continuous changes force a scan after about one second, and mutations during an active scan produce one serialized replay. Picks-tab capture accepts only strict, currently rendered numbered cards from the semantic Queue/Picks tab control; Queue entries are ignored. It never scrolls the panel automatically; virtualized windows accumulate as Yahoo renders them and on manual rescan. Return to **Results → Round by Round** and rescan when anything looks incomplete; its numbered table remains the only authoritative ledger and repair source. The extension sends a small exact `round-by-round` proof only after that source is verified. Before any player is selected, the empty table is usable only with a stable Yahoo current-pick marker showing pick 1. If an active scan sees no ledger, current-pick marker, or drafted-player observation, it syncs one integrity blocker; repeating the same empty scan does not advance the saved state time. Sessions created by older extension builds have no proof and remain blocked until a current authoritative rescan.
...
The dashboard shows the same clock-aware decision brief, market badges, take-now/can-wait reasoning, and bounded Sleeper Watch, plus a broader live cockpit: personal queue, position/tier filters, three-strategy sensitivity, recent position-run alerts, configured roster-slot gaps, fallback tiers, readiness checks, quick comparison, and a value/reach recap. Its **Live refresh** toggle is on by default. While visible, the page polls only an exact, bounded local revision—session identity, timestamp, counters, and capture-blocked status, never picks or player/team identities—then quietly coalesces rapid changes and scores only the newest revision. The prior cockpit and cards remain available while a refresh is computing; hidden pages pause, transient failures back off, and action-required failures pause for that revision until a profile/settings/manual change or the next pick. It also includes up to twenty detailed candidates, recent draft history, specialist scores, critic checks, simulations, and source/quality diagnostics. Its **Provider cache** panel shows metadata-only database counts, size, freshness, and local FantasyPros request reservations. Select `STD`, `HALF`, or `PPR` and run the TTL-aware FantasyPros + Sleeper cache job manually; fresh snapshots are reused, and a second job is rejected instead of queued. Expand the market definitions, trust checks, and bounded-exclusion details when auditing a label. It and the sidebar use text-only DOM rendering and never inject controls into Yahoo.
...
3. Confirm the league ID, team count, roster slots, and explicit Standard, Half PPR, or PPR selection.
...
5. Select **Import profile**. Recommendations refresh automatically after the exact profile is saved; the manual refresh button remains available.

The server keeps only the top 500 sanitized rows, roster settings, the exact selected scoring format, and safe provenance for that exact sport/league/team/session identity. The visible scoring selector overrides any workbook-inferred label. Raw workbook bytes are parsed in memory and discarded. Filenames, formulas, notes, URLs, and arbitrary cells are not stored. Older profiles without a scoring format remain readable and are labeled **Scoring not set** until explicitly reimported or reused.
...
A label appears only for fresh evidence at most 45 days old, a same-position/source/opportunity-kind cohort of at least five players, no more than three experience years, and projected points plus opportunities at or above the cohort's 60th percentile. It is marked uncalibrated, remains stable as players are drafted because the full bounded ranking cohort is used, and is never inferred from ADP or headlines. Missing evidence omits the label without degrading normal recommendations. FantasyPros news remains risk-only. Fresh FantasyPros preseason projections can form an automatic cohort when the server also finds a unique, conservative experience match in its daily cached Sleeper player catalog.
...
3. Under **Reuse a saved profile**, explicitly choose the prior source and scoring format.
4. Select **Use for this draft & refresh**.

For repeated mocks, the dashboard's **Default for future drafts** form can store one explicit saved source and scoring override per sport. A first recommendation for a new profileless recorder identity binds that source with the selected scoring before Yahoo fallback. Existing exact profiles always win, changing or clearing the default affects only future unbound drafts, and the server copies only sanitized rankings, roster settings, scoring, and provenance—never picks. An older scoring-less default can be upgraded by selecting the same source, choosing scoring, and saving it again; this changes only the private default pointer. The source must match the current UTC year when selected and when bound; replace or clear it after a season rollover.
...
The recorder also validates the Yahoo team ID, not just sport and league. If the same browser-storage key contains state from a different team, automatic scans, sync, and pending repair replay stop without changing either session. Open the original team's exact draft tab and reset it before recording the other team; the recorder never reclassifies the old team's picks or roster as the active team's state.
...
When `FANTASY_PROS_API` is set on the server, recommendations receive FantasyPros-attributed status, recent headlines, and cached preseason RB/WR/TE projection evidence. Matching candidate cards render the allowlisted projected points and position-appropriate opportunities as inert, attributed evidence with explicit season, scoring, freshness, and retrieval provenance. The extension never receives the key. FantasyPros does not supply experience years, so the server conservatively joins matching `years_exp` values from Sleeper's public NFL player catalog. A unique Yahoo ID plus position is authoritative; otherwise a unique exact name/position/team is required, with narrowly bounded one-suffix and `FA`-versus-missing-team fallbacks for non-initialed names. Conflicting, ambiguous, or unmatched stable identities remain unresolved instead of falling back loosely. When unresolved matches remain, data quality shows only aggregate matched/total counts for eligible RB/WR/TE ranking rows; QB/K/DST rows are not identity failures. The normalized Sleeper catalog and allowlisted canonical decimal-string cross-provider IDs are cached locally for 24 hours in the same provider-neutral SQLite database as the FantasyPros snapshots, and raw responses are not retained.

The first FantasyPros-enabled recommendation creates or safely migrates the private SQLite cache automatically; there is no separate prefetch step. Complete normalized catalog, locally selected scoring-specific projection, and scoring-specific ADP views last 24 hours; a provider-limited partial catalog and injury/news snapshots are retried after five minutes. The full enrichment phase has a 10-second outer deadline. An explicit imported scoring setting selects `points`, `points_half`, or `points_ppr` locally from FantasyPros' scoring-neutral projection feed; otherwise the selection reports a `HALF` default. A missing league season similarly uses the current UTC year with a warning. `STD` and `PPR` use their explicit same-season catalog ADP fields. `HALF` uses the official consensus-rankings `type=ADP` response and validates its `ADP Half PPR` label, without interpolation or ECR substitution. Projections do not establish NFL experience, so they cannot independently create a breakout label.
